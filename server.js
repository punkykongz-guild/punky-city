...
app.get("/api/profile", async (req, res) => {
  ...
  res.json({ ...profile, isAdmin: ADMIN_NAMES.indexOf(name) >= 0, tokenIds: toks.map((t) => t.id), imgMap, imageBase: "https://punkykongz.com/nft/punkykongz/image/" });
});
...
app.post("/api/idle/save", (req, res) => {
  ...
  const GAME_FIELDS = ["money", "lifetime", "stage", "levels", "souls", "avatarId", "lastSeen",
                       ...
                       "szMeta", "szBest", "szN", "szRun", "tapN", "tapLv", "bothLv", "pjN", "arAiN", "sfAiN", "sukaN", "sukaBest", "hexN", "hexBest", "rgN", "rgBest",
                       "skyN", "rnN", "tdN", "k2N", "k3N"]; // 일일한도 영속(새로고침 파밍 방지)
  const p = db.players[name] || (db.players[name] = {});
...
app.get("/api/admin/reset", (req, res) => {
  ...
  const GAME = { money: 0, lifetime: 0, stage: 1, souls: 0, rewardedStage: 1, lastCollectDate: "", avatarId: 0,
                 ...
                 moleN: 0, simonN: 0, skyN: 0, k2N: 0, k3N: 0, rnN: 0, tdN: 0, tapN: 0, pjN: 0, bnDate: "", bnCount: 0, mgDate: "",
...
function claudeAsk(req, res) { // GET(카톡봇 Jsoup)·POST 겸용
  ...
  const name = String(src.name || "").trim().normalize("NFC");
